...
// Simple SVG icons (you can replace these with more sophisticated ones)
const IctIcon = () => (
...
const SiteMap = () => {
    ...
    const [movableItems, setMovableItems] = useState({
        ...
        areaL5: [ {id: 'itemL5-1', label: 'FSCM ICT', x: 20, y: 10}, {id: 'itemL5-2', label: 'BMU ICT2', x: 73, y: 30}, {id: 'itemL5-3', label: 'BMU ICT1', x: 82, y: 30}, {id: 'itemL5-4', label: 'BMU Flashing', x: 64, y: 65}, {id: 'itemL5-5', label: 'Frontend ICT', x: 40, y: 65}, ],
         ...
         areaR1: [ {id: 'itemR1-1', label: 'Knor Aeroflex', x: 80, y: 45}, {id: 'itemR1-2', label: 'Knor Seica', x: 60, y: 45}, ],
        areaR2: [],
        areaR3: [],
        ...
        area6: [ {id: 'item6-1', label: 'MIX ICT1', x: 67, y: 95}, {id: 'item6-2', label: 'MIX ICT3', x: 42, y: 75}, {id: 'item6-3', label: 'MIX ICT4', x: 42, y: 47}, {id: 'item6-4', label: 'Flashing 1', x: 70, y: 5}, {id: 'item6-5', label: 'Flashing 2', x: 70, y: 30}, ],
    });
    ...
    const handleSelectAction = (action, item) => {
        console.log(`Action: ${action} selected for item: ${item.label} (ID: ${item.id})`);
        alert(`Action "${action}" triggered for "${item.label}"`);
             ...
             <div className="relative h-full w-full">
                 {areaItems.map(item => (
                     <button
                         ...
                         className={`absolute flex items-center justify-center rounded-full cursor-pointer hover:ring-2 ring-offset-1 ${itemColorClass} hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-offset-1 ${itemColorClass}`}
                         style={{
                ...
                {/* --- Action Menu --- */}
                 {actionMenu.visible && actionMenu.item && (
                    ...
                    >
                        ...
                        <button onClick={() => handleSelectAction('VNC', actionMenu.item)} className="block w-full text-left px-2 py-1 hover:bg-gray-100 rounded">Connect VNC</button>
                        <button onClick={() => handleSelectAction('Disk C', actionMenu.item)} className="block w-full text-left px-2 py-1 hover:bg-gray-100 rounded">Access Disk C</button>
                        <button onClick={() => handleSelectAction('Disk D', actionMenu.item)} className="block w-full text-left px-2 py-1 hover:bg-gray-100 rounded">Access Disk D</button>
                    </div>
                 )}
